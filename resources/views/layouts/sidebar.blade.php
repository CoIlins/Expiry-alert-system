    ...
    <nav class="flex-1 px-4 py-6 space-y-6 overflow-y-auto">
        ...
        <div>
            ...
            @endphp
                <a href="{{ route($dashboardRoute) }}" 
                   class="flex items-center px-3 py-2 text-sm font-semibold rounded-lg transition-all {{ request()->routeIs($dashboardRoute) ? 'bg-slate-800/60 text-indigo-400 font-semibold' : 'text-slate-400 hover:bg-slate-800/60 hover:text-slate-100'}}">
                    Dashboard
                </a>
        </div>
        ...
        @if(Auth::user()->role_id == 3)
            <div class="space-y-2">
                ...
                <span class="px-3 text-xs font-bold text-slate-500 uppercase tracking-wider block mb-2">Batch Management
